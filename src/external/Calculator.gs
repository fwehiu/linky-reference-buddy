...
function getData() {
  try {
    ...
    // Region data - now using single discount column (column J for discount)
...
function createGoogleDocReport(resultData, templateId) {
  try {
    ...
    Logger.log("Accessing template file, ID: " + templateId);
    const templateFile = DriveApp.getFileById(templateId);
    if (!templateFile) {
      throw new Error("Could not find template file.");
    }
    ...
    const copyId = googleDocCopy.getId();
    Logger.log("Google Doc copy created: ID " + copyId);
    ...
    const docUrl = googleDocCopy.getUrl();
    Logger.log("Returning Google Doc URL: " + docUrl);
    return { url: docUrl, success: true };
  } catch (e) {
    ...
    return { error: `Failed to create Google Doc: ${errorMessage}` };
  }
}
